...
class TestValidateConfig:
    ...
    def test_no_config_file_returns_missing_provider_and_model(
        self, config_service, mock_cyclon_paths
    ):
    ...
    def test_only_provider_returns_missing_model(self, config_service, mock_cyclon_paths):
        ...
        mock_cyclon_paths["local_config_file"].write_text(json.dumps({"provider": "test"}))
    ...
    def test_only_model_returns_missing_provider(self, config_service, mock_cyclon_paths):
        ...
        mock_cyclon_paths["local_config_file"].write_text(json.dumps({"model": "gpt-4"}))
...
class TestLoadProviders:
    ...
    def test_valid_json_returns_dict(
        self, config_service, mock_cyclon_paths, sample_providers_data
    ):
        """Test that valid JSON returns parsed dict."""
        mock_cyclon_paths["providers_file"].write_text(json.dumps(sample_providers_data))
...
class TestBuildCommand:
    ...
    def test_explicit_provider_parameter(self, config_service, mock_cyclon_paths):
        ...
        providers = {"explicit": {"name": "Explicit", "command": ["explicit-cmd"]}}
        mock_cyclon_paths["providers_file"].write_text(json.dumps(providers))
        mock_cyclon_paths["local_config_file"].write_text(
    ...
    def test_oserror_on_read_raises_config_error(self, config_service, mock_cyclon_paths):
        ...
        with patch("builtins.open", side_effect=OSError("Permission denied")):
            ...
            assert "Error reading providers file" in str(
                exc_info.value
            ) or "Error reading config" in str(exc_info.value)
