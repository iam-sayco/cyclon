...
from cyclon.widgets import (
    TerminalOutput, StatusIndicator, Throbber, ProcessStatus, 
    PromptInput, TopAlignedAutoComplete, CommandSuggester
)
from cyclon.screens import (
    CommandsModalScreen, ProviderModalScreen, ModelModalScreen, FileModalScreen
)
from cyclon.constants import COMMANDS_LIST, VALID_COMMANDS
from cyclon.paths import (
    PROJECT_ROOT, PROMPTS_DIR,
    LOCAL_CYCLON_DIR, LOCAL_CONFIG_FILE, LOCAL_PLAN_FILE, LOCAL_PROMPT_FILE,
    LOCAL_LOCK_FILE
)
...
class CyclonApp(App):
    ...
    def __init__(self, config_service: ConfigService, file_service: FileService, process_service: ProcessService, app_state: AppState | None = None):
        super().__init__()
    ...
    async def generate_plan(self, user_prompt: str) -> None:
        ...
        if not command:
            self.append_output("[red]Error: Could not build command. Check provider/model config.\n")
            return
        ...
        try:
            ...
            if returncode == 0:
                ...
                self.append_output("Use [b]/plan[/b] to view or edit, then [b]/run[/b] to begin execution.\n")
            else:
                self.append_output(f"\n[red]Generation failed with return code {returncode}[/red]\n")
        
        except FileNotFoundError:
            self.append_output("[red]Provider command not found. Ensure provider is installed and in PATH.[/red]\n")
    ...
    async def run_processing_loop(self):
        ...
        while True:
            ...
            try:
                self.append_output(f"[bold yellow]→ Provider running — check Provider Output for live logs[/bold yellow]\n")

                returncode, output = await self.process_service.run_provider_process(command, timeout)

                self.current_process = None

                if returncode != 0:
                    self.append_output(f"[red]Provider failed with return code {returncode}[/red]\n")

                    if self.check_exception(output):
                        self.append_output(f"[yellow]Exception matched, sleeping for {sleep_duration} seconds...[/yellow]\n")
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(classes="main-container"):
            ...
            with Horizontal(classes="output-grid", id="output-grid"):
                with Vertical(classes="log-column"):
                    ...
                    with Container(classes="log-box", id="log-container"):
                        yield RichLog(id="app-log", wrap=True, highlight=True, auto_scroll=True, markup=True, max_lines=10000)
            ...
            yield TopAlignedAutoComplete(
                self.prompt_input,
                candidates=COMMANDS_LIST
            )
            yield Static("[b]F1[/b] - Commands | [b]Ctrl+Q[/b] - Exit", classes="info-bar", id="info-bar")
    ...
    def _validate_command_state(self, command: str) -> bool:
        ...
        if command == "/run":
            if not LOCAL_PLAN_FILE.exists():
                ...
                self.append_output("Generate a plan with your description or use [b]/plan[/b] to create one.\n\n")
                return False
        elif command == "/stop":
            if not self.current_process and not self.process_service.current_pty:
    ...
    def _execute_command(self, command: str, argument: str) -> None:
        ...
        if command == "/stop":
            ...
            self.push_screen(ProviderModalScreen(config_service=self.config_service), self.on_provider_modal_dismissed)
        elif command == "/model":
            current_model = ""
            ...
            self.push_screen(ModelModalScreen(current_model=current_model), self.on_model_modal_dismissed)
            ...
            if argument:
                ...
                self.push_screen(FileModalScreen(file_name="plan.md", modal_title="Edit Plan", file_service=self.file_service), self.on_file_modal_dismissed)
        elif command == "/prompt":
            self.push_screen(FileModalScreen(file_name="prompt.md", modal_title="Edit Prompt", file_service=self.file_service), self.on_file_modal_dismissed)
        elif command == "/run":
    ...
    async def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        if not text.startswith("/"):
            self.append_output(f"[red]Error: Unknown command '{text}'. Commands must start with '/'. Type /help or press F1 for available commands.[/red]\n")
            return
        
        command, argument = self._parse_command(text)
        
        if command not in VALID_COMMANDS:
            self.append_output(f"[red]Error: Unknown command '{command}'. Type /help or press F1 for available commands.[/red]\n")
            return
    ...
    async def send_input_to_process(self):
    ...
    def _prepare_execution(self, mode: str = "start") -> bool:
        ...
        self.append_output("[yellow]Starting processing... (observe Provider Output for live logs)[/yellow]\n\n")
