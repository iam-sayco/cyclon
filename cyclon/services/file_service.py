...
from cyclon.paths import (
    LOCAL_CYCLON_DIR, LOCAL_PLAN_FILE, LOCAL_PROMPT_FILE, LOCAL_LOCK_FILE,
    LOGO_FILE, PROMPTS_DIR
)
from cyclon.exceptions import FileError
from cyclon.constants import (
    CONFIG_FILENAME, PLAN_FILENAME, PROMPT_FILENAME,
    LOCK_FILENAME, CONTEXT_FILENAME
)
from cyclon.validation import validate_string, validate_not_none
...
class FileService:
    ...
    def save_file(self, file_name: str, content: str) -> None:
        ...
        try:
            ...
            raise FileError(f"Error saving file {file_name}: {e}")
    ...
    def save_plan(self, plan_content: str) -> None:
        ...
        try:
            ...
            raise FileError(f"Error saving plan file: {e}")
    ...
    def load_logo(self) -> str:
        ...
        if LOGO_FILE.exists():
            try:
                ...
                raise FileError(f"Error reading logo file: {e}")
        return "CYCLON"
    ...
    def load_generate_prompt(self, user_prompt: str) -> str:
        ...
        try:
            ...
            raise FileError(f"Error resolving plan file path: {e}")
        ...
        if prompts_file.exists():
            try:
                ...
                raise FileError(f"Error reading prompt template: {e}")
        else:
    ...
    def build_execution_prompt(self, plan_content: str | None = None) -> str:
        ...
        try:
            ...
            raise FileError(f"Error resolving file paths: {e}")
        ...
        if LOCAL_PROMPT_FILE.exists():
            try:
                ...
                raise FileError(f"Error reading prompt file: {e}")
        ...
        if plan_content is None:
            try:
                ...
                raise FileError(f"Error reading plan file: {e}")
    ...
    def clear_session(self) -> None:
        ...
        try:
            ...
            raise FileError(f"Error clearing session: {e}")
    
    def load_content(self, file_name: str) -> Optional[str]:
        ...
        if file_path.exists():
            try:
                ...
                raise FileError(f"Error reading file {file_name}: {e}")
        return None
    ...
    def create_lock_file(self) -> None:
        ...
        try:
            ...
            raise FileError(f"Error creating lock file: {e}")
    ...
    def remove_lock_file(self) -> None:
        ...
        try:
            ...
            raise FileError(f"Error removing lock file: {e}")
    ...
    def remove_context_file(self) -> None:
        ...
        try:
            ...
            raise FileError(f"Error removing context file: {e}")
