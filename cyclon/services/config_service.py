...
from cyclon.paths import (
    PROVIDERS_FILE, DEFAULT_CONFIG_FILE, LOCAL_CONFIG_FILE, LOCAL_CYCLON_DIR
)
from cyclon.exceptions import ConfigError
from cyclon.constants import CONFIG_FILENAME
from cyclon.validation import validate_string, validate_optional_string
...
class ConfigService:
    ...
    def get_status_message(self) -> str:
        ...
        if config_exists:
            ...
            if provider:
                ...
                status_lines.append("[orange1]●[/orange1] [orange1]Provider not set - use /provider[/orange1]")
            
            if model:
                status_lines.append(f"[green]●[/green] Model: [b]{model}[/b]")
            else:
                status_lines.append("[orange1]●[/orange1] [orange1]Model not set - use /model[/orange1]")
        else:
    ...
    def load_providers(self) -> Dict[str, Any]:
        ...
        if PROVIDERS_FILE.exists():
            try:
                ...
                raise ConfigError(f"Invalid JSON in providers file: {e}")
            except OSError as e:
                raise ConfigError(f"Error reading providers file: {e}")
        return {}
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        if DEFAULT_CONFIG_FILE.exists():
            try:
                ...
                raise ConfigError(f"Error reading default config: {e}")
        ...
        if LOCAL_CONFIG_FILE.exists():
            try:
                ...
                raise ConfigError(f"Error reading local config: {e}")
    ...
    def _save_json_config(self, key: str, value: str) -> None:
        ...
        try:
            ...
            raise ConfigError(f"Invalid JSON in existing config file: {e}")
    ...
    def build_command(self, provider: str | None = None, model: str | None = None, prompt: str | None = None) -> List[str]:
        ...
        if provider is None and LOCAL_CONFIG_FILE.exists():
            try:
                ...
                raise ConfigError(f"Error reading config for command building: {e}")
        ...
        result_cmd = []
        for part in cmd:
            if model_placeholder in part and model:
                part = part.replace(model_placeholder, model)
            if "{prompt}" in part and prompt:
                part = part.replace("{prompt}", prompt)
            result_cmd.append(part)
